# main.py
# 200 samples — with binned trace length analysis and sequence-level Levenshtein
...
# Bins derived from trace length distribution (Q1=9, Q3=16, 90th=23)
BIN_EDGES = [0, 9, 16, 23, float("inf")]
    ...
    for retrieval_mode in RETRIEVAL_MODES:
        ...
        for seed in SEEDS:
            ...
            for N_FEWSHOT in FEWSHOT_SETTINGS:
                ...
                # Per-bin accumulators: correct counts, violation counts, total counts
                bin_correct = {b: 0 for b in BIN_LABELS}
                bin_violations = {b: 0 for b in BIN_LABELS}
                bin_total = {b: 0 for b in BIN_LABELS}
                ...
                for idx_ex, (prefix, target) in enumerate(examples):
                    ...
                    # Bin is determined by the full trace length at this prediction point
                    trace_len = len(prefix) + 1
                    length_bin = get_length_bin(trace_len)
...
"""
                    ...
                    prompt = f"""
You are predicting the next event in a hospital treatment process.
...
"""
                    ...
                    bin_total[length_bin] += 1
                    if is_correct:
                        bin_correct[length_bin] += 1
                    if not is_valid:
                        bin_violations[length_bin] += 1
                ...
                # Attach per-bin accuracy and GVR to each row
                for b in BIN_LABELS:
                    total = bin_total[b]
                    row[f"acc_{b}"] = bin_correct[b] / total if total > 0 else None
                    row[f"gvr_{b}"] = bin_violations[b] / total if total > 0 else None
                    row[f"n_{b}"] = total

                all_metrics.append(row)


# Aggregate overall results across seeds
...
# Aggregate binned results across seeds

bin_agg_rows = []

for (model, retrieval, fewshot), group in df.groupby(["model", "retrieval", "fewshot"]):
    row = {"model": model, "retrieval": retrieval, "fewshot": fewshot}
    for b in BIN_LABELS:
        valid_acc = group[f"acc_{b}"].dropna()
        valid_gvr = group[f"gvr_{b}"].dropna()
        row[f"acc_{b}_mean"] = valid_acc.mean() if len(valid_acc) > 0 else None
        row[f"acc_{b}_std"] = valid_acc.std() if len(valid_acc) > 0 else None
        row[f"gvr_{b}_mean"] = valid_gvr.mean() if len(valid_gvr) > 0 else None
        row[f"gvr_{b}_std"] = valid_gvr.std() if len(valid_gvr) > 0 else None
        row[f"n_{b}_total"] = group[f"n_{b}"].sum()
    bin_agg_rows.append(row)

bin_agg = pd.DataFrame(bin_agg_rows)

print("\nBinned Results:")
print(bin_agg)

bin_agg.to_csv("binned_results.csv", index=False)
print("\nSaved: binned_results.csv")